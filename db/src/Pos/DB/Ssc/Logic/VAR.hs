...
sscVerifyBlocks coreConfig bvd blocks = do
...
sscApplyBlocks
    ...
    -> m [SomeBatchOp]
sscApplyBlocks coreConfig bvd blocks (Just newState) = do
    inAssertMode $ do
        let hashes = map headerHash blocks
        expectedState <- sscVerifyValidBlocks coreConfig bvd blocks
        if | newState == expectedState -> pass
           | otherwise -> onUnexpectedVerify hashes
    sscApplyBlocksFinish newState
sscApplyBlocks coreConfig bvd blocks Nothing =
    sscApplyBlocksFinish =<< sscVerifyValidBlocks coreConfig bvd blocks
...
sscVerifyValidBlocks
    ...
    -> OldestFirst NE SscBlock
    -> m SscGlobalState
sscVerifyValidBlocks coreConfig bvd blocks =
    sscVerifyBlocks coreConfig bvd blocks >>= \case
